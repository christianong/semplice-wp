	...
	class module_api {
		...
		public function background_video($styles, $visibility) {
			...
			if($this->detect->isMobile()) {
				$fallback_image = ' poster="' . $this->background_video_fallback_image($styles) . '"';	
			}
			// has bg video?
			...
			if(!empty($bg_video) && $visibility == 'frontend') {
				if(!$this->detect->isMobile() || $this->detect->isMobile() && $force_autoplay == 'enabled') {
					$video = '
						<div class="background-video">
							<video data-autoplay autoplay webkit-playsinline playsinline loop muted data-object-fit="cover"' . $fallback_image . '>
		...
		public function background_video_fallback($styles, $id, $visibility) {
			...
			$force_autoplay = isset($styles['bg_force_autoplay']) ? $styles['bg_force_autoplay'] : 'disabled';
			// get fallback image
			$fallback_image = $this->background_video_fallback_image($styles);
			// is fallback image there?
			if(false !== $fallback_image) {
				// only show in editor or mobile frontend
				if($visibility == 'editor' || $this->detect->isMobile() && $force_autoplay == 'disabled') {
					// css code
		...
		public function background_video_fallback_image($styles) {
			// vars
			$image = false;
